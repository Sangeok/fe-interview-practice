...
// Interview option metadata with colors, icons, and descriptions
const OPTION_META = {
  Subjective: {
    color: "from-orange-400 via-amber-500 to-orange-500",
    glow: "shadow-[0_0_30px_rgba(251,146,60,0.5)]",
    icon: "✍",
    desc: "Open-ended responses",
    accent: "orange-400",
  },
  "Multiple Choice": {
    color: "from-purple-400 via-violet-500 to-purple-500",
    glow: "shadow-[0_0_30px_rgba(168,85,247,0.5)]",
    icon: "✓",
    desc: "Quick knowledge test",
    accent: "violet-400",
  },
  "Custom Questions": {
    color: "from-pink-400 via-rose-500 to-pink-500",
    glow: "shadow-[0_0_30px_rgba(244,114,182,0.5)]",
    icon: "⚡",
    desc: "Your custom set",
    accent: "pink-400",
  },
};

export default function InterviewOptions({ selectedOptions, setSelectedOptions, onClose }: InterviewOptionsProps) {
  const [hoveredCard, setHoveredCard] = useState<InterviewOptionsValue | null>(null);
  ...
  return (
    <div role="radiogroup" aria-label="인터뷰 옵션 선택" className="relative p-6">
      {/* Header */}
      <div className="mb-6 text-center">
        <div className="inline-flex items-center gap-2 mb-2">
          <div className="w-1.5 h-1.5 rounded-full bg-lime-400 animate-pulse"></div>
          <span className="text-lime-400 font-mono text-xs tracking-wider">MODE SELECTION</span>
          <div className="w-1.5 h-1.5 rounded-full bg-lime-400 animate-pulse"></div>
        </div>
        <h3 className="text-zinc-400 text-sm font-mono">
          <span className="text-cyan-400">{">"}</span> Choose your interview format
        </h3>
      </div>

      {/* Option Cards Grid */}
      <div className="grid grid-cols-1 gap-4 mb-6">
        {INTERVIEW_OPTIONS.map((option, index) => {
          const meta = OPTION_META[option.value as keyof typeof OPTION_META];
          ...
          return (
            <button
              ...
              className={`
                ...
                ${isSelected ? "scale-[1.02]" : "scale-100 hover:scale-[1.01]"}
              ...
              style={{
                ...
                transform: isHovered && !isSelected ? "perspective(1000px) rotateY(2deg) rotateX(-2deg)" : "none",
              }}
            >
              {/* Holographic border */}
              <div
      ...
      <div className="flex justify-between items-center gap-3">
        <button
          onClick={handleSelectInterviewOption}
